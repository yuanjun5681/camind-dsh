...
- 扩展 `SessionEventMap`：`cam/stage`（阶段推进）、`cam/check-report`（自检结论）、`cam/delivered`（交付包清单）——持久、可回放。**三个事件的 Host 侧 append 已落地（2026-08-23，run.js / deliver.js）；卡片渲染器与「交付物」页签接线已实现（2026-08-23，P3 前半）**；
- **⚠️ 已发现的持久化隐患（P1 遗留，2026-08-23 核实源码）**：`session.append()` 的公开签名无法给事件打 `ignorable: true` 标记（只接受 surface 元数据），而 `dsh-session-persistence` 的 `assertEventsSupported` 在加载/检查/导出路径拒绝「不在本构建 KNOWN_SESSION_EVENT_TYPES 且未标 ignorable」的事件类型——上游原注「下游插件事件的注册面推迟到有消费者再做」。后果：**含 cam/* 事件的会话日志在进程重启后拒绝重载**（live 期间一切正常，事件流/卡片均不受影响）。插件侧无规避手段，需上游提供注册面或 append 支持 ignorable；在此之前，cam 会话的历史价值以 run 目录留档为准；
- **卡片（已实现，`tool-cam/lib/client.js`）**：注册 3 个 `conversationEvents` 定义 + keyed `conversation.chat.node` 渲染器——阶段卡（cam-stage：阶段推进 + 逐序状态点；兼吃同 run 的 cam/check-report 做终结——run.js 中途失败 failStage/取消只发 check-report(error) 不发 done 阶段，不终结阶段卡会永远停在「进行中」）、检查报告卡（cam-check-report：overall 徽章 + 逐工序终态 + NC 对账/空刀路摘要；cam/check-report 载荷无逐序数据，定义同时折叠同 run 的 cam/stage op 落定事件）、交付卡（cam-delivered：交付包清单 + NC 程序列表 + 下载/打开 + 刀路挂点）。**收敛语义刻意全 role=update、`buildViewNode` 从 `context.matches` 纯折叠**（不拆 start/update）：cam_run resume 以同一 run_id 重发整轮 cam/stage（ensure_ready 开头），start 语义会撞 runtime 的「more than one start Match」不变量；全折叠免疫，且「同一会话同一 run 一条可更新卡」跨续跑成立（锚点随最新一轮执行移动）——与官方定义的 fallbackState 从 matches 重放是同一条路的常态形态；
- **「交付物」页签接线（已实现）**：官方 deliverables 投影（dsh-client-ui-deliverables）从 pending call 视图收集 `card:'generic' + kind:'edit' + locations` 的产出路径（turn 级，`turn.data['deliverables'].produced`）——`cam_deliver` 声明 `presentCall` 列出三件套的工作区相对路径（`delivery/<runId>/`），并把交付三件套镜像落进会话工作区（设计稿 §3 决策 4；best-effort，失败只记 note），ui-shell 现成的 turnTail chips 与「交付物」页签/预览弹层自动生效（预览路由以会话工作区为界）。已知取舍：cam_deliver 的错误是返回 error JSON 而非抛错（isError=false），传输级失败的交付也会登记 produced 路径，文件不在盘上时预览报「文件不存在」；
...
- **刀路挂点（已实现）**：交付卡声明 keyed slot `cam.nc.preview`（scope root），有 `toolpath-viewer` 注册才显示「查看刀路」（点击经下载路由取 NC 文本，以 owner props `{ content, fileName }` 渲染席位）；无注册退化为文件清单。
- **工作台「加工」页签（已实现 2026-08-23，`ui-shell/src/web/CamRuns.tsx` + `tool-cam/lib/runs-route.js`）**：ui-shell Workbench 第三页签，按 session 列 CAM run（run_id/件号/机床/overall 徽章/工序状态点/更新时间，展开见 runstate 全文与工序明细）——**run 列表从 run 目录落盘读**（新路由 `GET /camind/api/cam/runs?session=<id>` 列表 + `GET /camind/api/cam/runs/<session>/<runId>` 详情，与 delivery 下载同一 prefix、同一白名单/防越界纪律），不读会话事件投影：免疫上面记录的「cam 事件会话重启后拒绝重载」上游限制，也能展示中断后可 resume 续跑的 run（无 runstate → planned 徽章 + 首跑提示，incomplete/error → resume 提示行）。交付文件下载走既有 delivery 路由；报告/设定单可经会话工作区镜像走全局预览弹层；「查看刀路」按钮取 NC 文本后直接渲染 slot 注册表里 `cam.nc.preview`/`toolpath-viewer` 的组件（ui-shell 是 slot runtime 宿主且与官方 bundle 同一 React 实例，viewer 只消费 owner props，故无需逃生门、viewer 本体零改动；页签不重复声明席位——slot 系统对重复声明抛错）。
...
3. **P3 会话卡片渲染器 + 刀路查看器 + 设置页「NX 工作台」**（§4.5 设置页已完成）；**P4 经验库扩展**。P3 拆两个插件（2026-08-23 决策）：会话卡片（预检清单/检查报告/交付卡）挂在 tool-cam 的 client bundle；**刀路查看器独立为 `camind-ui-toolpath-viewer`**（NC 解析 + WebGL 回放 + keyed 渲染器席位）——职责是呈现而非执行、vendor 资产（旧 Camind `viewer_assets` 的 cnc-simulator，244KB）与构建形态不同、消费方不止交付卡一处（交付物页签、将来报价预览）、WebGL 失败可独立降级。tool-cam 交付卡经 slot 弱耦合消费：viewer 在则渲染回放，不在退化为文件清单。**查看器已实现（2026-08-23）**，要点：
   ...
   - **渲染器**：自写最小 WebGL lines（约 250 行）：单 interleaved 顶点缓冲（position+color）一次 `gl.LINES` 绘制、深度测试、Z-up 球轨道（拖动旋转/滚轮缩放/Shift 或右键平移）、按需重绘（无 RAF 循环）、DPR 自适应、包围盒线框 + 原点 RGB 轴三联；配色快移红/切削蓝/圆弧青/孔位琥珀，固定深色视口；WebGL 创建失败降级为中文说明面板（消费方仍可退化文件清单）。
