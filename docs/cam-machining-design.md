...
- **「交付物」页签接线（已实现）**：官方 deliverables 投影（dsh-client-ui-deliverables）从 pending call 视图收集 `card:'generic' + kind:'edit' + locations` 的产出路径（turn 级，`turn.data['deliverables'].produced`）——`cam_deliver` 声明 `presentCall` 列出三件套的工作区相对路径（`delivery/<runId>/`），并把交付三件套镜像落进会话工作区（设计稿 §3 决策 4；best-effort，失败只记 note），ui-shell 现成的 turnTail chips 与「交付物」页签/工作台各「预览」入口自动生效（预览路由以会话工作区为界，内容显示在主对话区「预览」标签页）。已知取舍：cam_deliver 的错误是返回 error JSON 而非抛错（isError=false），传输级失败的交付也会登记 produced 路径，文件不在盘上时预览报「文件不存在」；
