...
function extractZip(bytes, archive, batchDir, mediaTypeOf) {
    if (!hasZipSignature(bytes))
        throw new Error(`ZIP 文件头无效：${archive.original_name}`);
    ...
    const unzip = new Unzip((entry) => {
        ...
        try {
            ...
            if (seen.has(relative))
                throw new Error(`ZIP 包含重复路径：${entry.name}`);
            seen.add(relative);
            entryCount += 1;
            if (entryCount > MAX_ARCHIVE_FILES)
                throw new Error(`ZIP 文件数超过限制 ${MAX_ARCHIVE_FILES}`);
        ...
        entry.ondata = (error, chunk, final) => {
            if (error && !failure)
                failure = error;
            if (failure) {
                if (fd !== undefined && openFiles.delete(fd))
                    closeSync(fd);
                return;
            }
            size += chunk.length;
...
export async function saveUploadBatch(sessionId, inputs, mediaTypeOf) {
    if (!inputs?.length)
        return null;
    if (inputs.length > MAX_UPLOAD_FILES)
    ...
    try {
        ...
        for (const [index, input] of inputs.entries()) {
            if (!input?.name || typeof input.data !== 'string')
                continue;
            const bytes = decodeBase64(input.data);
            if (bytes.length > MAX_UPLOAD_BYTES)
                throw new Error(`文件过大：${input.name}`);
            ...
            files.push(stored);
            if (archive)
                extractedFiles.push(...extractZip(bytes, stored, batchDir, mediaTypeOf));
        }
        const manifest = {
            schema_version: 2,
            batch_id: batchId,
...
export function availableUploads(manifest) {
...
function uploadManifestAt(batchDir, sessionId, batchId) {
    try {
        const parsed = JSON.parse(readFileSync(path.join(batchDir, 'manifest.json'), 'utf8'));
...
function readPendingPaths(batchDir, manifest) {
    ...
    try {
        ...
        return new Set(parsed.paths.filter((candidate) => (typeof candidate === 'string' && known.has(candidate))));
    }
    catch {
...
function writePendingSelection(batchDir, createdAt, paths) {
    const selection = {
...
function selectPendingFiles(manifest, paths) {
    return {
        ...manifest,
        files: manifest.files.filter((file) => paths.has(`upload://${manifest.batch_id}/files/${file.stored_name}`)),
        extracted_files: manifest.extracted_files.filter((file) => paths.has(`upload://${manifest.batch_id}/${file.stored_path}`)),
    };
}
/** 把一个已完整提交的批次标记为等待下一条用户消息消费。 */
export function markUploadBatchPending(manifest) {
 ...
 */
export function pendingUploadBatches(sessionId) {
    const normalizedSessionId = safeSessionId(sessionId);
    ...
    for (const entry of readdirSync(sessionDir, { withFileTypes: true })) {
        if (!entry.isDirectory() || !UPLOAD_BATCH_RE.test(entry.name))
            continue;
...
export function listSessionUploads(sessionId) {
    const normalizedSessionId = safeSessionId(sessionId);
    const sessionDir = path.join(UPLOADS_ROOT, normalizedSessionId);
    if (!existsSync(sessionDir))
        return [];
...
export function removePendingUpload(sessionId, batchId, requestedPath) {
    const normalizedSessionId = safeSessionId(sessionId);
    if (!UPLOAD_BATCH_RE.test(batchId))
        throw new Error('上传批次无效');
    const batchDir = path.join(UPLOADS_ROOT, normalizedSessionId, batchId);
    const manifest = uploadManifestAt(batchDir, normalizedSessionId, batchId);
    if (!manifest)
        throw new Error('上传批次不存在或已损坏');
    const selectedPaths = readPendingPaths(batchDir, manifest);
    if (!selectedPaths || !selectedPaths.has(requestedPath))
        throw new Error('待发送文件不存在');
    selectedPaths.delete(requestedPath);
    const direct = manifest.files.find((file) => (requestedPath === `upload://${manifest.batch_id}/files/${file.stored_name}`));
    if (direct?.archive) {
...
export function consumePendingUploadBatches(sessionId, batchIds) {
...
export async function resolveUploadReference(sessionId, requested) {
    ...
    if (!rootInfo.isDirectory())
        throw new Error(`上传批次不存在：${batchId}`);
    const info = await stat(candidate);
    if (!info.isFile())
        throw new Error('上传引用不是文件');
