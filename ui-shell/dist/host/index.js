...
import { availableUploads, consumePendingUploadBatches, listSessionUploads, markUploadBatchPending, pendingUploadBatches, removePendingUpload, resolveUploadReference, saveUploadBatch, } from './uploads.js';
...
function createPrompt(text, images = []) {
    const content = [];
    if (text)
        content.push({ type: 'text', text });
    for (const attachment of images) {
        content.push({ type: 'image', attachment: Object.freeze({ ...attachment }) });
    }
    if (content.length === 0)
...
function uploadContextText(batches) {
    return [
...
function createUploadContext(batches) {
...
function previewKind(mediaType) {
    if (mediaType.startsWith('text/') || mediaType === 'application/json' || mediaType === 'image/svg+xml')
        return 'text';
...
function bindModelSelection(agentCtx, selection) {
...
export function apply(ctx) {
    ...
    ctx.on('session/event', (...args) => {
        ...
        if (event.type === 'session/title') {
            const title = event.data?.title;
            if (title)
                push(session.id, { type: 'title', title });
        }
    });
    ...
    ctx.on('agent/pre-step', async (...args) => {
        const payload = args[0];
        const next = args.at(-1);
        if (!payload.messages.some(isDirectUserMessage))
            return next();
        let batches;
        try {
        ...
        if (decision.kind === 'reject' || payload.signal.aborted || !decision.messages)
            return decision;
        try {
    ...
    async function resolveSelection(requested) {
        if (requested)
            return requested;
        const fallback = ctx.agentDefaultModel?.currentSelection();
        ...
        if (!group || !model)
            throw new Error('没有可用模型：请先在官方设置里配置 Provider');
        return { provider: group.id, model: model.id };
    }
    function permissionSelect(events = []) {
        ...
        if (currentValue === 'custom')
            options.push(presets.optionOf('custom'));
        return { options, currentValue };
    }
    function applyPermission(session, preset) {
        if (!preset || !ctx.permissionPresets)
            return;
        if (preset === 'custom')
            return;
    ...
    function lastUserPromptAt(events) {
        let latest = null;
        for (const event of events) {
    ...
    function isUserInvocable(item) {
        return item.invocation?.userInvocable !== false;
    }
    function skillRegistry(agent) {
        const scoped = ctx.agentPresets?.serviceFor(agent, 'skills');
        return scoped ?? ctx.skills;
    }
    function listUpdatedAt(createdAt, events) {
    ...
    async function listPresetCatalog() {
    ...
    async function presetRef(id) {
        if (!id || !ctx.agentPresets)
            return undefined;
        try {
    ...
    async function resolvePresetId(sessionId, requested) {
        const presets = ctx.agentPresets;
        if (!presets)
            return undefined;
        if (requested)
    ...
    async function spawnAgent(kind, sessionId, cwd, requested, requestedPreset) {
        ...
        if (presetId)
            mountedPresets.set(sessionId, presetId);
        return handle.agent;
    }
    async function ensureLive(sessionId, requested) {
        const live = ctx.agents.get(sessionId);
        if (live) {
            const existing = selections.get(sessionId);
            if (existing && requested)
                existing.current = requested;
            return live;
        }
    ...
    async function readSessionState(sessionId) {
        ...
        if (live)
            return { id: live.id, header: live.header, events: [...live.events] };
        if (!persist)
            return undefined;
        try {
    ...
    async function forkSession(sessionId) {
        ...
        if (presetId)
            mountedPresets.set(childId, presetId);
    ...
    async function resolveSessionFile(sessionId, requested) {
        ...
        async function resolveRelative(requestedPath) {
    ...
    async function buildState() {
        ...
        for (const session of ctx.sessions.list())
            known.set(session.id, session.header);
        for (const header of persisted) {
            if (!known.has(header.id))
                known.set(header.id, header);
        }
        for (const [id, header] of known) {
            if (accounted.has(id) || archived.has(id))
                continue;
    ...
    async function snapshotOf(sessionId) {
        ...
        if (!persist)
            throw new Error('会话不在内存中，且没有持久化后端');
        const inspection = await persist.inspect(sessionId);
    ...
    async function handleApi(req, res, url) {
        ...
        if (await handleSettingsApi(ctx, req, res, url))
            return;
        ...
        if (method === 'POST' && pathname === '/camind/api/pick-directory') {
            ...
            try {
                ...
                sendJson(res, 200, { kind: 'native', path: picked });
            }
            catch (err) {
                if (abort.signal.aborted) {
                    sendJson(res, 200, { kind: 'native', path: null });
                    return;
                }
        ...
        if (method === 'POST' && pathname === '/camind/api/sessions') {
            ...
            const agent = await spawnAgent('create', sessionId, workspace.path, requestedModel(body), typeof body.agentPreset === 'string' ? body.agentPreset : undefined);
        ...
        if (method === 'POST' && action === 'files') {
            let body;
            try {
                body = await readJson(req, MAX_UPLOAD_REQUEST_BYTES);
            }
            catch (error) {
        ...
        if (method === 'GET' && action === 'stream') {
            ...
            req.on('close', drop);
            try {
                sseWrite(res, { type: 'snapshot', snapshot: await snapshotOf(sessionId) });
                const agent = ctx.agents.get(sessionId);
                if (agent)
                    sseWrite(res, { type: 'status', status: agent.status });
            }
            catch (err) {
