...
ui-preview/         内容预览插件（`camind-ui-preview`）：Host 会话文件预览路由
                    ...
                    （ui-shell 等插件的预览入口）；仅 web profile
ui-sidebar/         /camind 专用的官方 Sidebar 兼容实现；扩展底部菜单 owner props
ui-brand/           Camind 品牌插件（`camind-ui-brand`）：CAM 虎钳工匠 mascot + 字标，
                    注册到上游原生 sidebar.brand.mark/name 席位
...
ui-toolpath-viewer/ NC 刀路查看器（`camind-ui-toolpath-viewer`，设计稿 §7 P3）：
                    ...
                    （含时间轴动画：画布满幅、HUD/播放条叠加、源码抽屉、快移/切削图层、俯视/等轴）；
...
tool-cam/           CAM 加工插件（`camind-tool-cam`，设计稿 docs/cam-machining-design.md）：
                    ...
                    断点续跑 + NC 对账/空刀路自检 + runstate.history 过程时间线落盘）
                    + cam_deliver 交付打包（delivery/ 三件套 + 工作区 nc/ 实数开包：nc_batch.zip 开包实数
                    对账 + 交付报告 + 加工设定单 + 镜像进会话工作区 delivery/<runId>/
                    [presentCall 喂官方 deliverables 投影]）
                    ...
                    cam/* 会话事件与聊天三卡已于 2026-08-26 停发退役（会话重启
                    拒绝重载事故实证；历史日志用 scripts/repair-cam-session-events.mjs
                    标 ignorable 修复）；
                    所有模式全局加载
...
- **ui-brand**（`camind-ui-brand`）— Camind 品牌插件：Host 以 exact 路由 `/camind/assets/camind-mascot.png` 提供选定的 CAM 虎钳工匠 PNG；手写 client bundle 注册到上游原生 `sidebar.brand.mark` / `sidebar.brand.name` single 席位（priority -10 压过图内 ui-brand-official 的 priority 0 注册；仅 `/camind` 路径注册，官方壳保持原生品牌）。展开态渲染 mascot + “Camind” 字标与主题反白 “Harness” 徽章（deepseek-HARNESS 风格），折叠 rail 只渲染 mascot。`ui-home` 经 `BrandLockup` 复用同一图片。仅 web profile。
...
- **ui-toolpath-viewer**（`camind-ui-toolpath-viewer`）— NC 刀路查看器（设计稿 `docs/cam-machining-design.md` §7 P3；v1 只看轨迹，不做材料去除仿真）。向 keyed slot `cam.nc.preview`（消费方如 tool-cam 交付卡声明，scope root，owner props `{ content, fileName }`）注册 key `toolpath-viewer` 的渲染器；经 `ctx.slots.inject` 等席位声明，无消费方时整体惰性（不挂 /camind 路径守卫——官方壳不声明该 slot）。解析器与渲染器均为自写，不用旧 Camind `viewer_assets` 的 cnc-simulator（parseGcode.js/RenderPath.js 是 GPL-3.0-or-later，`asset_licenses.json` 标 `CONDITIONAL_GPL_PATH`、外部授权 false；且 shell 共享包无 three.js）：`lib/nc-parser.js` 是纯函数零依赖 ESM（Fanuc 方言：G0/G1/G2/G3 模态、G17/18/19 平面[G17 精确]、G90/G91、G20/G21 只记录、圆弧弦差细分[含 R 形/整圆/螺旋]、G73/G74/G76/G81..G89 固定循环展开为到孔/下 R/进给/退刀（G83 按 Q 啄钻并快退 R，G73 高速啄钻回退 1 mm，G74/G84/G85/G89 进给退，G76/G87/G88 仍为单条 R→Z 并告警）、G28/G30 只画中间点、坏行整行跳过计数、S0/G20/截断写入 warnings、段数上限 100 万），node 可直接 import 验证；client.js 单文件 bundle 内联同一份字节（PARSER CORE 标记区间保持同步——手写 bundle 不能相对 require，dsh 模块加载器只认 seed 词与裸包名）。渲染器是最小 WebGL lines（单 interleaved 顶点缓冲一次 gl.LINES、Z-up 球轨道[拖动旋转/滚轮缩放/Shift 或右键平移]、按需重绘、DPR 自适应；WebGL 创建失败降级为说明面板），快移红/切削蓝/圆弧青/循环琥珀 + 包围盒线框与原点轴三联；时间轴动画（参考旧 Camind viewer 的 setStopAtTime 语义自写实现，无 GPL 代码：段计时 = 距离/模态 F[解析器每段快照 feed 字段；快移按固定假定速率 8000/未设 F 按 500，viewer 级近似]）；画布满幅，播放条与 XYZ/行号 HUD 叠在视区上，进度用比例（估算时间只作 tooltip）；已走段变淡、当前段加亮、未走更淡；快移/切削图层可关；俯视/等轴/复位；点 3D 段或告警芯片打开右侧源码抽屉并对到 NC 行（点源码跳刀位仍可用）；空格播放、左右键单步。免责声明在标题 tooltip，不占高度。仅 web profile。
...
2. `http://127.0.0.1:3080/`（302 到 `/camind/`）：验证自定义前端（需先在 `ui-shell/` 执行 `npm install && npm run build`）。新会话页 `/` 显示 Camind 品牌区（CAM 虎钳工匠 mascot + 字标 + Harness 徽章）、紧随其下的示例卡片，与官方工作区行/输入卡组成一个整体居中组，官方鱼标/「探索未至之境」/预览徽章不再出现（ui-home 叠加 + CSS 隐藏）；点示例直接写入输入框草稿（无会话时先自动接最近工作区的 blank 会话再预填）。侧栏顶部显示同一 mascot + 字标（ui-brand 占 `sidebar.brand.mark`/`sidebar.brand.name` 席位；折叠 rail 只剩 mascot），底部「设置」上方显示「记忆库」（page-memory），点击进入 `/camind/pages/memory`：知识/经验双 tab 列表、搜索与 chip 过滤、上传 .md/.txt 后元数据自动生成（「生成中」→ 自动填好或「生成失败」可编辑手填）、编辑/删除、经验采纳/弃用，详情深链接刷新保持。Composer 加号打开斜杠命令列表（含命令与技能），输入 `/` 同样弹出。官方壳由 `http://127.0.0.1:3080/web`（302 到 `/index.html`）访问，保持原生品牌与布局。
